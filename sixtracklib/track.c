...
static inline int Drift_track(Particles* p, uint64_t ip,
                                double length){
  double xp, yp;
  xp = p->px[ip] * p->rpp[ip];
  yp = p->py[ip] * p->rpp[ip];
...
static inline int DriftExact_track(Particles* p, uint64_t ip, 
                                    double length){
...
static inline int Multipole_track(Particles* p, uint64_t ip, 
                                long int order, double l, 
                                double hxl, double hyl, double* bal){
...
static inline int Cavity_track(Particles* p, uint64_t ip,
                double volt, double freq, double lag){

  double phase, pt, opd, beta;
  phase=lag-2*M_PI/CLIGHT*freq*p->sigma[ip]/p->beta0;
  p->psigma[ip]+=p->chi[ip]*volt*sin(phase)/(p->p0c*p->beta0);
  pt=p->psigma[ip] * p->beta0;
  opd=sqrt( pt*pt+ 2*p->psigma[ip] + 1 );
  p->delta[ip]=opd - 1;
  beta=opd/(1/p->beta0+pt);

  p->rpp[ip]=1/opd;
  p->rvv[ip]=p->beta0/beta;
  
  return 1;
}


static inline int Align_track(Particles* p, uint64_t ip,
